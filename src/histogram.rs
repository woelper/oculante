...
impl ArgumentReducer<f32, u16> for ArgumentReducerFloat32 {
    ...
    fn bins(&self, min: f32, _max: f32, range: f32) -> Vec<f32> {
        ...
        (0..u16::MAX as i32 + 1)
            .map(|f| min + f as f32 * norm_factor)
            .collect()
    }
}
    ...
    V: AsPrimitive<usize> + AsPrimitive<u32>,
{
    ...
    let range_scale: A;
    if USE_MIN_MAX {
        min_scale = min_value.mini(A::RANGE_MIN);
        max_scale = max_value.maxi(A::RANGE_MAX);
    } else {
    ...
    for chunk in image.chunks_exact(CN) {
        ...
        for i in 0..USEFUL_CN {
            let c: usize = reducer
                .reduce(chunk[i], min_scale, max_scale, range_scale)
                .as_();
            hist_bins[i][c] += 1;
            trans &= chunk[i].is_transparent_alpha();

            let col_reduced: u8 =
                distinct_colors_reducer.reduce(chunk[i], min_scale, max_scale, range_scale);
            asdf[i] = col_reduced;
        }
        if has_alpha {
