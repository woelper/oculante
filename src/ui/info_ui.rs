...
use notan::egui::style::ScrollStyle;
...
pub fn info_ui(ctx: &Context, state: &mut OculanteState, _gfx: &mut Graphics) -> (Rect, f32) {
    ...
    let sp = egui::SidePanel::left("info")
        .show_separator_line(false)
        .min_width(PANEL_WIDTH)
        .frame(
            egui::Frame::central_panel(&ctx.style())
                .corner_radius(CornerRadius::ZERO)
                .fill(Color32::from_rgba_unmultiplied(100, 100, 0, 0)),
        )
        .show(ctx, |ui| {
    ...
    (bbox, sp.response.rect.width())
}

fn info_grid(ui: &mut Ui, state: &mut OculanteState, color_type: ColorType) {
    let uv_center        = (
        state.cursor_relative.x as f64
            / state.image_geometry.dimensions.0 as f64,
        (state.cursor_relative.y as f64
            / state.image_geometry.dimensions.1 as f64),
    );
    
    egui::Grid::new("info").num_columns(2).show(ui, |ui| {
        ui.label_i(format!("{ARROWS_OUT} Size",));
        ui.label_right(RichText::new(format!(
            "{}x{}",
        ...
        if let Some(path) = &state.current_path {
            ...
            let path_label = egui::Label::new(RichText::new(file_name)).truncate();
        ...
        ui.label_right(RichText::new(disp_col(state.sampled_color)));
        ui.end_row();

        ui.label_i(format!("{PALETTE} RGBA"));
        ui.label_right(RichText::new(disp_col_norm(state.sampled_color, 255.)));
        ui.end_row();

        ui.label_i(format!("{PALETTE} HEX"));
        let hex = Color32::from_rgba_unmultiplied(
        ...
        .to_hex();
        ui.label_right(RichText::new(hex));
        ui.end_row();

        ui.label_i(format!("{PALETTE} Color"));
        ui.label_right(format!("{:?}", color_type));
        ui.end_row();
...
fn advanced_ui(ui: &mut Ui, state: &mut OculanteState) {
    if let Some(info) = &state.image_metadata {
        ...
        Plot::new("histogram")
            ...
            .show_axes(false)
            .show_grid(false)
            // .width(50. - ui.style().spacing.scroll.bar_width)
